...
const ActivationsOoredoo = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            let q;
            const activationsRef = collection(db, "ActivationsOoredoo");
    ...
    return (
        <Box sx={{ height: '100%', width: '100%', p: 2 }}>
            ...
            <Stack direction="row" spacing={2} mb={3} alignItems="center" sx={{ bgcolor: '#fff', p: 2, borderRadius: 2, boxShadow: 1 }}>
                ...
                />
                <Button variant="contained" startIcon={<SearchIcon />} onClick={fetchData} sx={{ bgcolor: '#ed1c24' }}>
                    Filtrer
                </Button>
            ...
            <Dialog open={openExport} onClose={() => {setOpenExport(false); setExportStep(0);}} fullWidth maxWidth="md">
                ...
                <DialogContent dividers>
                    ...
                    {exportStep === 2 && (
                        <Box>
                            <TextField label="Titre du tableau" fullWidth sx={{ mb: 3 }} placeholder="Ex: Offre Nkalmek Mois Avril 2026"
            ...
            <Dialog open={open} onClose={() => { setOpen(false); resetForm(); }} fullWidth maxWidth="sm">
                ...
                <DialogContent dividers>
                    ...
                    {activeStep === 1 && (
                        <Box sx={{ p: 3, bgcolor: '#ffebee', borderRadius: 2, border: '1px solid #ef9a9a' }}>
                    ...
                    {activeStep === 2 && (
                        <Stack spacing={3}>
                            <TextField 
                                label="Numéro Ooredoo" fullWidth value={formData.numero}
                ...
                <DialogActions sx={{ p: 2 }}>
                    ...
                    {activeStep === 0 && <Button variant="contained" onClick={handleCheckClient} sx={{ bgcolor: '#ed1c24' }}>Suivant</Button>}
                    {activeStep === 1 && <><Button onClick={() => setActiveStep(0)}>Retour</Button><Button variant="contained" onClick={() => setActiveStep(2)} sx={{ bgcolor: '#ed1c24' }}>Confirmer</Button></>}
                    {activeStep === 2 && <><Button onClick={() => setActiveStep(1)}>Retour</Button><Button variant="contained" onClick={handleSaveActivation} sx={{ bgcolor: '#ed1c24' }}>Enregistrer</Button></>}
                </DialogActions>
            </Dialog>
        </Box>
    );
