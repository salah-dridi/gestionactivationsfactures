...
const FacturesPayees = () => {
    ...
    const handleSaveFacture = async () => {
        ...
        try {
            await addDoc(collection(db, "FacturesPayees"), formData);
            
            Swal.fire('Succès', 'Facture payée enregistrée', 'success');
            setOpen(false);
            resetForm();
            fetchData();
        } catch (error) {
    ...
    return (
        <Box sx={{ height: '100%', width: '100%', p: 2 }}>
            ...
            <Dialog open={open} onClose={() => { setOpen(false); resetForm(); }} fullWidth maxWidth="sm">
                <DialogTitle>Enregistrer une Facture Payée</DialogTitle>
                <DialogContent dividers>
                    ...
                    {activeStep === 1 && (
                        <Box sx={{ p: 3, bgcolor: '#e8eaf6', borderRadius: 2, border: '1px solid #1a237e' }}>
                    ...
                    {activeStep === 2 && (
                        <Stack spacing={3}>
                            ...
                            />
                            <TextField 
                                label="Montant Payé (DT)" fullWidth type="number"
                                value={formData.montant}
                                onChange={(e) => setFormData({...formData, montant: e.target.value})}
                            />
                            <TextField 
                                label="Date de Paiement" type="date" fullWidth 
                                InputLabelProps={{ shrink: true }}
                                sx={dateFieldStyle}
                                value={formData.date}
